...
- The image tag is derived from `rust-toolchain.toml`, the generated Anvil
  recipe tree, the Containerfile, and optional auth-hook source.
...
mounts the token read-only only while running `anvil-aprz`, then runs the
remaining requested checks without the token and removes the temporary file.
If `gh` is installed but not authenticated, an interactive run
...
Hook source is part of the image hash because it can define non-secret build
customization. Runtime tokens and secret-file contents are not hashed.
