...
| `bolero`  | Uses the catalog nightly consistently for target discovery and execution, then runs each affected libfuzzer target for 60 seconds on Linux. A successful empty discovery is a no-op; discovery or parsing failure fails the check. Non-Linux hosts skip because cargo-bolero's native dependencies are unsupported there, while harnesses still run as ordinary tests. | oxidizer-github |
