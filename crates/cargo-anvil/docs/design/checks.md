...
| `spellcheck`                   | `cargo spellcheck check --code 1`. Self-skips on aarch64, where cargo-spellcheck ships no installable binary (x86_64-only releases; the source fallback needs libclang the hosted ARM images lack). Spelling is a property of the source tree, not the host, so the x86_64 legs give complete coverage. The skip covers setup and validation too, so aarch64 legs never install the tool. | oxidizer-github |
