...
`_anvil-impact-format` helper — into one selector token per line: `--package`, a version-qualified workspace package
spec, and so on, or `--none` when the tier is empty. Unscoped tiers resolve to
`--workspace`. The portable `name@version` specs are unambiguous for both native Cargo
and cargo-each consumers.
