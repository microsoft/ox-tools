...
| `pr-title`                     | Conventional-Commits validation applied to the title in the `PR_TITLE` env var. Invalid titles fail with the accepted title patterns and case-insensitive type names so the author can correct the title directly. Skipped when the env var is **unset or empty** (intermediate local runs, scheduled-tier builds, and the best-effort ADO fallback described below); a non-empty-but-invalid value fails loudly (a misconfigured cloud variable is a real error, not a skip). Written as a `[script("pwsh")]` recipe (the one check that needs scripting; see [README.md §8.3](./README.md#83-cross-os-test-matrices)). cloud-workflow wiring: GitHub's per-group composite action reads `${{ inputs.pr_title }}` populated from `${{ github.event.pull_request.title }}` in the reusable PR workflow; ADO has **no** PR-title predefined variable (`System.PullRequest.Title` does not exist), so `group.yml` resolves the title from the REST API on PR builds and publishes it as `PR_TITLE` (empty on non-PR / fork / API failure, where the recipe no-ops). | oxidizer-github |
...
| `semver-check`                 | `cargo semver-checks --baseline-rev <baseline>` per affected library crate (`--workspace` fails on bin-only members). The PR's target branch is the CI baseline; local runs use `origin/main` or `origin/master`. The recipe skips crates whose current manifest path is absent from the baseline and tolerates renamed or bin→lib crates with no comparable baseline. Findings are **advisory**: it writes `target/anvil/comments/semver.md` for the cloud sticky comment and exits 0. | oxidizer-github |
...
| Check        | Invocation                                                                  | Source |
|--------------|-----------------------------------------------------------------------------|--------|
| `llvm-cov`   | Instruments and runs the affected tests **once per feature config** (`--all-features`, then `--no-default-features`), so coverage reflects both feature-gated code and default-only paths. Each config runs as a self-contained `cargo +<pinned-nightly> llvm-cov nextest --no-report` preceded by `llvm-cov clean`, then emits its own `report --lcov` → `target/coverage/lcov-<config>.info` and `report --cobertura` → `target/coverage/cobertura-<config>.xml`. Per-config reports (rather than one union report) keep each `report`'s `--object=` set small enough to stay under the Windows `CreateProcess` command-line limit; the configs are reconciled downstream -- `cargo-coverage-gate` merges the lcov files at line level, Codecov ingests both, and ADO's `PublishCodeCoverageResults@2` coalesces both cobertura files. Runs on **nightly** so `cargo-llvm-cov`'s `cfg(coverage_nightly)` gate activates the `#[cfg_attr(coverage_nightly, coverage(off))]` exclusions (inert on stable, which would miscount untestable code). The recipe then runs `cargo coverage-gate --lcov …lcov-all-features.info --lcov …lcov-no-default.info`, which merges both and fails the check when a package's line coverage is below its threshold (`[package.metadata.coverage-gate] min-lines-percent`, then `[workspace.metadata.coverage-gate]`, then `100.0`). Codecov is display-only; the gate is authoritative. | oxidizer, oxidizer-github; gate via [`cargo-coverage-gate`](../../../cargo-coverage-gate) |
