...
| `unique-target-names`          | Reads `cargo metadata --no-deps` and fails when two workspace packages own targets that uplift to the same file. Cargo compiles each unit into `deps/` under a metadata-hashed name, then uplifts the root unit to `target/<profile>/` (examples to `target/<profile>/examples/`) under a plain, unhashed name, so duplicates overwrite each other. Targets are keyed by **every** file they uplift, because the crate-type-to-file relationship runs both ways: `cdylib`, `dylib`, and `proc-macro` all emit the platform shared library and share a key, while an executable and a shared library emit different primary files (`tool.exe`, `tool.dll`) yet both write `tool.pdb`. Files whose name derives from another artifact -- the import library and export file beside a Windows DLL, and the `<artifact>.cargo-sbom.json` sidecar -- are covered by that artifact's key and are not keyed separately. The debug-info file is keyed separately because its name does not derive that way: it is `<name>.pdb`, not `<name>.exe.pdb`, so nothing else pins it down. An `rlib` and a `staticlib` emit no uplifted debug-info file, so they coexist with a binary of the same name. The Windows debug-info file is keyed on every platform so a build matrix agrees across legs. An ordinary `lib` is included -- packages `foo-bar` and `foo_bar` both uplift to `libfoo_bar.rlib` with no configuration anywhere, since Cargo normalises `-` to `_` in the default library target name. Only test and benchmark binaries and build scripts stay in `deps/` and are exempt. Keys and report order are ordinal, matching Cargo's own path-keyed check and keeping the diagnostic identical under every host culture. Duplicates inside a single package are already rejected by Cargo. A `[script("pwsh", "-NoProfile")]` recipe; there is no cargo subcommand for this. | oxidizer |
