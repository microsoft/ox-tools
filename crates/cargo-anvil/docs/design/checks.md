...
| Check     | Invocation                                                                                                                                                           | Source |
|-----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------|
| `miri`    | `cargo +<pinned-nightly> miri test --all-features --tests` over the impact-affected packages. Uses libtest (one process per test binary), **not** `cargo miri nextest run`. Miri's interpreter is single-threaded -- it emulates libtest's threads rather than running them -- so this run uses exactly one core however large the runner is. That is deliberate, and two ways of buying parallelism have been measured and rejected. nextest's process-per-test model makes every test re-pay miri's interpretation of the binary's startup, which on a large suite is 10-25s *per test*; measured end-to-end it cost ~3x the CPU to buy ~1.3x the wall-time, because libtest also amortizes interpreter state (monomorphizations, warmed allocator) across the tests in a binary in a way process-per-test cannot. Fanning the packages out into concurrent `cargo miri test --package` invocations does not work either: cargo holds the build-directory lock for the whole command, including the test-run phase, so the invocations serialize and only one miri process is ever runnable. Real parallelism would require building with `--no-run` and then invoking the test binaries through `cargo-miri runner` directly, bypassing cargo -- not worth the coupling to cargo-miri internals. `--tests` runs lib/bin unit tests and integration tests while excluding doctests, which miri can't run; it is used in preference to `--lib --tests` because `--lib` errors with "no library targets found" on a bin-only affected package under impact scoping. libtest exits 0 when a binary's tests are all skipped, so no `--no-tests=pass` workaround is needed. The recipe defaults to `--workspace` locally when no impact env vars are set, so plain `just anvil-pr-runtime-analysis` runs the full miri suite. | oxidizer, oxidizer-github |

Because the run is single-core and serial, **the per-test opt-out is the lever that actually controls miri's wall-clock**. Miri cost is not spread evenly: it concentrates in a small tail of tests whose purpose is *scale* (deep nesting, long chains, large populations) or *exhaustive sweeps* (every registry entry, every table row, algebraic laws over a generated corpus). Those tests multiply a cost miri already inflates 100-1000x, while exercising no unsafe code, so miri learns nothing from them. Mark them `#[cfg_attr(miri, ignore = "<reason>")]`, stating in the reason both what makes the test expensive and why miri has nothing to find. Tests that reach the host -- clock, filesystem, process spawning -- must carry the same opt-out, because miri's isolation turns them into a hard `unsupported operation` abort that kills the whole test binary rather than a single failure. anvil doesn't pass exotic `MIRIFLAGS`; the per-test opt-out is the canonical mechanism.
