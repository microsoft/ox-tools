...
| `semver-check`                 | `cargo semver-checks --baseline-rev <PR target branch>` per library crate (per-package because `--workspace` fails on bin-only members, and we tolerate "package `<name>` not found in ..." / "no library targets found" for new/renamed or bin→lib-transition crates). The baseline is the PR's target branch (`origin/<base>`, or `origin/main`/`origin/master` locally) — resolved via the shared `_anvil-base-ref` recipe — so the check reports what **this PR** changes rather than the accumulated delta since the last crates.io release. The GitHub `pr-fast` job therefore checks out with `fetch-depth: 0` so the baseline commit is resolvable locally (ADO's default checkout already has full history). **Advisory only**: findings do not fail the recipe (breaking changes between unreleased commits are normal — the major-version bump happens at release time, not on every PR). Instead the recipe writes a markdown body to `target/anvil/comments/semver.md` when there are findings and removes the file when the tree is clean; cloud-workflow wiring turns presence/absence into a sticky PR comment (see §6 below). | oxidizer-github |
