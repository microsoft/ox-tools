...
| `unique-target-names`          | Reads `cargo metadata --no-deps` and fails when two workspace packages declare targets that uplift to the same file. Cargo compiles each unit into `deps/` under a metadata-hashed name, then uplifts the root unit to `target/<profile>/` (examples to `target/<profile>/examples/`) under a plain, unhashed name, so duplicates share one file -- plus one import library and debug-info file on Windows, and one SBOM sidecar when CI generates them. Targets are grouped by emitted file name rather than crate type: `cdylib`, `dylib`, and `proc-macro` all emit the platform shared-library file and share a bucket, while an `rlib` and a `cdylib` of the same name emit different files and do not. An ordinary `lib` is included -- packages `foo-bar` and `foo_bar` both uplift to `libfoo_bar.rlib` with no explicit configuration anywhere. Only test and benchmark binaries and build scripts stay in `deps/` and are exempt. Keys compare ordinally, matching Cargo's own path-keyed check, so names differing only in case are not merged. Duplicates inside a single package are already rejected by Cargo. A `[script("pwsh", "-NoProfile")]` recipe; there is no cargo subcommand for this. | oxidizer |
