...
| `doc-test`   | Two cargo-test runs over affected packages with library targets: `cargo test --doc --all-features --locked` and `cargo test --doc --locked` (default features). Explicit selections exclude binary-only packages and retain library and proc-macro packages with their version-qualified specs. An all-binary selection is a logged skip; workspace-wide runs retain `--workspace`. The metadata probe uses `--locked`; metadata failures and invalid selections remain errors. Running both feature configurations catches doctests that only compile under one configuration. nextest does not run doctests, so this stays a separate cargo-test invocation. | oxidizer, oxidizer-github |
