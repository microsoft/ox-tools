...
| `unique-target-names`          | Reads `cargo metadata --no-deps` and fails when two workspace packages declare the same uplifted target name. Cargo copies binaries, examples, and `cdylib` / `dylib` / `staticlib` libraries out of `deps/` into `target/<profile>/` (examples into `target/<profile>/examples/`) without a metadata hash, so duplicates share one file -- plus one debug-info file on Windows, and one SBOM sidecar when CI generates them. Targets are grouped by the file name they emit rather than by crate type, so a `cdylib` and a `dylib` of the same name are caught while a `staticlib` alongside either is not (they emit different names). Cargo reports `output filename collision`, warns it may become a hard error, and builds anyway. Test and benchmark binaries, build scripts, and `lib` / `rlib` / `proc-macro` libraries keep their metadata hash in `deps/`, so duplicate names there are not reported. Duplicates inside a single package are already rejected by Cargo. A `[script("pwsh", "-NoProfile")]` recipe; there is no cargo subcommand for this. | oxidizer |
