...
| `spellcheck`                   | `cargo spellcheck check --code 1`. Self-skips on aarch64, where cargo-spellcheck ships no installable binary (x86_64-only releases; the source fallback needs libclang the hosted ARM images lack). Spelling is a property of the source tree, not the host, so the x86_64 legs give complete coverage. The paired `anvil-tool-cargo-spellcheck-install` / `-validate-prereqs` recipes self-skip on the same hosts, so aarch64 legs never install the tool and direct invocation of the tool recipes is safe there too. | oxidizer-github |
