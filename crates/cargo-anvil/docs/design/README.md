...
|-------------------------------------------------------------|----------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------|
| `pr-fast`, `scheduled-advisories`                             | All legs above (`spellcheck` self-skips on the two aarch64 legs) | Contain compile-sensitive checks (clippy, doc-build, udeps, semver-check, external-types) that only see the host's compiled crate graph -- cfg-gated code is invisible to a single-leg run. Text/metadata checks running redundantly is cheaper than splitting jobs. cargo-spellcheck ships x86_64-only binaries, so the check self-skips on aarch64; spelling is a property of the source tree, so the x86_64 legs cover it. |
