...
| `pr-test`, `pr-runtime-analysis`, `scheduled-test`                     | All legs above                         | Where compile-time and runtime OS / arch bugs actually surface. The slow PR groups run as parallel cloud-workflow jobs for shorter wall-clock per leg. |
| `pr-msrv`                                                               | Linux x86_64                            | Compatibility tests under the declared MSRV when a selecting toolchain file uses a different compiler. |
