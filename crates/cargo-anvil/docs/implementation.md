...
`_anvil-impact-format` reverse-maps them to version-qualified workspace package selectors using three
...
affected work. The cache keeps portable `name@version` specs that cargo-each and
native Cargo commands consume directly. The impact state key includes a projection-format
