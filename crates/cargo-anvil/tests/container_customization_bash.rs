...
fn pr_recipes_run_without_github_token_plumbing() {
    ...
    // isolated APRZ invocation -- the negative half of the
    // `anvil_recipe_needs_github_token` case arm, otherwise pinned only by
    // generated-text snapshots. Accidentally re-adding any of the three recipes
    // to that arm would flip these assertions.
    let tmp = repo_with_container();
    let run = run_driver(
        tmp.path(),
        "",
    ...
    assert!(
        token_source_paths(&run.docker_log).is_empty(),
        "a PR recipe must not mount a GitHub token: {}",
        run.docker_log
    );
    assert_eq!(
        run.docker_log
