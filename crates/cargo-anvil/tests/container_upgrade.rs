...
//! Consumer-upgrade coverage for the container backend.
//!
...
//!
//! Across that upgrade `Containerfile` and its ignore file, `README.md`,
//! `entrypoint.sh`, `image-id.{sh,ps1}` and `run-in-container.{sh,ps1}` all
//! disappear from `.anvil/container/`, `justfiles/anvil/runner.just`
//! disappears with them, and the root `Justfile` loses its `anvil-runner`
//! region. What matters is that an untouched asset is removed cleanly and an
//! *edited* one is handed back to the repository rather than deleted.
...
/// Generated container assets that 0.4.0 tracked and the current catalog does
/// not. Paths are as 0.4.0 wrote them.
...
/// assets present on disk and tracked in the lock, and none of the current
...
fn upgrading_from_the_runner_layout_retires_the_seam_and_emits_the_new_backend() {
    ...
    // The current artifacts take their place and are tracked.
    for artifact in artifacts::container::all() {
        match artifact {
...
/// The upgrade re-seed replaces a whole-file render anvil produced. It
...
/// The refusal is atomic across *both* removal loops: a refused host keeps its
/// file and its lock entries whichever loop reaches them. A lock entry naming a
/// region the catalog does not declare would otherwise reach `remove_region`,
...
///
/// Reaching that path needs a retired region id, which the catalog has none of,
/// so the stale key is planted here. It is one region rename away from being
/// an ordinary upgrade.
#[cfg_attr(miri, ignore = "uses filesystem; miri isolation forbids it")]
#[test]
