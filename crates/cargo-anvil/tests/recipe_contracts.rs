...
_anvil-impact-include tier:
    $value = if ($env:FAKE_INCLUDE) { $env:FAKE_INCLUDE } else { '--workspace' }
...
fn impact_format_resolves_directory_aliases_and_fails_hard() {
    ...
    assert!(
        String::from_utf8_lossy(&directory_alias.stdout)
            .lines()
            .eq(["--package", "fixture-package@0.1.0"])
    );
...
fn build_and_doc_options_produce_the_expected_cargo_commands() {
    ...
    assert!(
        docs_commands.contains("each --workspace --once -- cargo doc {packages} --all-features --no-deps --open"),
...
fn llvm_cov_qualifies_direct_cargo_selectors_but_not_gate_selectors() {
    ...
    if calls.contains("nextest run") && !calls.contains("llvm-cov nextest") {
        assert!(
