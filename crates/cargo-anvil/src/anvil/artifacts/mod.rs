...
pub(crate) fn anvil_artifacts() -> Vec<Artifact> {
    ...
    out.extend(justfile::impact());
    out.extend(justfile::tiers());

    // Independently addressable recipe sections for every developer utility,
    // check, and group.
...
mod tests {
    ...
    fn every_registry_entry_is_in_the_anvil_catalog() {
        ...
        for artifact in justfile::tools()
            .iter()
            .chain(justfile::versions().iter())
            .chain(justfile::helpers().iter())
            .chain(justfile::impact().iter())
            .chain(justfile::tiers().iter())
            .chain(justfile::dev_files().iter())
        ...
        for artifact in anvil_artifacts() {
            if let Artifact::OwnedFile(spec) = artifact {
                ...
                let customizable_wrapper =
                    spec.path == ".anvil/ado/steps/job.yml" && spec.body.contains("Default job wrapper emitted by cargo-anvil.");
                assert!(
