    ...
    fn every_owned_file_identifies_cargo_anvil_as_its_generator() {
        for artifact in anvil_artifacts() {
            if let Artifact::OwnedFile(spec) = artifact {
                ...
                // The container image definition is deliberately editable in
                // place: a repository that needs a different base or extra
                // packages changes it and anvil's drift handling preserves the
                // edit. A "DO NOT EDIT" marker would contradict that, so these
                // carry the weaker provenance marker instead.
                let editable_image_definition =
                    spec.path.starts_with(".anvil/container/Dockerfile") && spec.body.contains("Managed by cargo-anvil.");
                assert!(
                    generated_marker || customizable_wrapper || editable_image_definition,
