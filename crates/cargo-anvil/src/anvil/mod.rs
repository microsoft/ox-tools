...
impl Catalog {
    ...
    /// shim, devcontainer, image-build recipes and Kind cluster harness).
    ///
    ...
    /// `anvil.toml` turns them on (`[container] enabled = true`, plus the
    /// relevant `[[image]]` / `[cluster]` sections). With containerization
