...
//!
//! `cargo-anvil` operates from the *workspace root*: the directory whose
...
pub struct Workspace {
    ...
    ///
    /// Used by the emitter (see the [design doc]): multi-crate workspaces get
...
pub fn find_workspace_root(start: &Path) -> Result<PathBuf, AppError> {
    ...
    nearest.ok_or_else(|| {
        app_err!(
            "no Cargo.toml found at or above '{}'; cargo-anvil must be run inside a Cargo workspace",
            start.display()
        )
    })
}
