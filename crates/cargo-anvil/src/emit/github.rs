...
//!
//! 1. Composite actions under `.github/actions/anvil-*/action.yml`.
//! 2. Reusable workflows (`anvil-pr-impl.yml`, `anvil-scheduled-impl.yml`).
//! 3. Root workflows (`anvil-pr.yml`, `anvil-scheduled.yml`).
//!
...
///
/// All anvil groups that get a per-group composite action.
///
...
/// variables, and invokes `just anvil-<group>`.
#[must_use]
...
pub fn plan_reusable_workflows(repo_root: &Path, manifest: &Manifest) -> Result<Vec<PlanItem>, AppError> {
    Ok(vec![
        plan_owned_file(repo_root, manifest, ".github/workflows/anvil-pr-impl.yml", PR_IMPL_WORKFLOW)?,
        plan_owned_file(
            repo_root,
            manifest,
...
pub fn plan_root_workflows(repo_root: &Path, manifest: &Manifest) -> Result<Vec<PlanItem>, AppError> {
    Ok(vec![
        plan_owned_file(repo_root, manifest, ".github/workflows/anvil-pr.yml", PR_ROOT_WORKFLOW)?,
        plan_owned_file(
            repo_root,
            manifest,
...
mod tests {
    ...
    fn group_action_passes_group_to_setup() {
        ...
        // The per-group composite invokes anvil-setup with its own
    ...
    fn pr_impl_workflow_has_expected_jobs() {
        ...
        // pr-fast carries the PR title for the anvil-pr-title check.
