...
//! Local recipe emission (the `justfiles/anvil/` tree).
//!
//! Each owned file under `justfiles/anvil/` is embedded at compile time
//! via [`include_str!`] from the `templates/justfiles/anvil/` directory.
...
/// recipe files and defines `alias anvil := anvil-pr`.
...
/// Emit a [`PlanItem`] for `justfiles/anvil/mod.just`.
///
/// # Errors
///
...
/// Emit a [`PlanItem`] for `justfiles/anvil/tools.just`.
///
/// # Errors
///
/// Propagates I/O errors from [`plan_owned_file`].
...
/// Emit a [`PlanItem`] for `justfiles/anvil/versions.just`.
///
/// # Errors
///
/// Propagates I/O errors from [`plan_owned_file`].
...
/// Emit a [`PlanItem`] for `justfiles/anvil/checks.just`.
///
/// # Errors
///
/// Propagates I/O errors from [`plan_owned_file`].
...
/// Emit a [`PlanItem`] for `justfiles/anvil/groups.just`.
///
/// # Errors
///
/// Propagates I/O errors from [`plan_owned_file`].
...
/// Emit a [`PlanItem`] for `justfiles/anvil/tiers.just`.
///
/// # Errors
///
/// Propagates I/O errors from [`plan_owned_file`].
...
/// file on disk, but never written by anvil; new files always use
...
///
/// - Fresh repos: anvil writes `Justfile` (canonical).
...
///
/// Earlier versions of cargo-anvil used `JUSTFILE_PATH = "justfile"`
...
/// last-rendered hash, and splice the region out — destroying anvil
...
/// Plan all five files of the `justfiles/anvil/` tree.
///
/// # Errors
///
/// Propagates I/O errors from any per-file emitter.
/// Plan all files of the `justfiles/anvil/` tree (recipes + data).
///
/// # Errors
///
/// Propagates I/O errors from any per-file emitter.
...
mod tests {
    ...
    fn groups_just_template_includes_all_groups_and_pr_slow_sub_recipes() {
        ...
        // The old pr-slow1/2/3 names should no longer appear.
    ...
    fn justfile_imports_writes_into_empty_repo() {
        ...
        assert!(spliced.contains("# >>> anvil-managed: anvil-imports"));
        assert!(spliced.contains("import 'justfiles/anvil/mod.just'"));
        // The alias lives in mod.just, not in the user's Justfile.
        assert!(!spliced.contains("alias anvil"));
    }
    ...
    fn matching_file_is_in_sync() {
        let tmp = TempDir::new().unwrap();
        std::fs::create_dir_all(tmp.path().join("justfiles/anvil")).unwrap();
