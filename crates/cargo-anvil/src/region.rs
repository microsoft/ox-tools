...
mod tests {
    ...
    #[test]
    fn an_exact_closer_reopens_scanning_for_a_final_unmanaged_table() {
    ...
    fn finds_region_with_body() {
        ...
        assert_eq!(region.start_line, ByteRange { start: 7, end: 36 });
        assert_eq!(region.body, ByteRange { start: 36, end: 60 });
        assert_eq!(region.end_line, ByteRange { start: 60, end: 89 });
    }
    ...
    #[test]
    fn at_placement_past_eof_is_clamped_to_eof() {
        let new = upsert_region_with_placement("FROM base\n", "x", "body\n", SYN, RegionPlacement::At(usize::MAX)).unwrap();
    ...
    #[test]
    fn at_placement_uses_the_end_of_the_prefix_to_detect_a_blank_separator() {
        let host = "\n\nFROM base\nRUN later\n";
        let offset = host.find("RUN later").expect("fixture line exists");
        let new = upsert_region_with_placement(host, "x", "body\n", SYN, RegionPlacement::At(offset)).unwrap();
        assert_eq!(
            new,
            "\n\nFROM base\n\n# >>> anvil-managed: x\nbody\n# <<< anvil-managed: x\n\nRUN later\n"
    ...
    #[test]
    fn at_placement_preserves_an_existing_blank_separator() {
        let host = "FROM base\n\nRUN later\n";
        let offset = host.find("RUN later").expect("fixture line exists");
        let new = upsert_region_with_placement(host, "x", "body\n", SYN, RegionPlacement::At(offset)).unwrap();
        assert_eq!(
    ...
    #[test]
    fn remove_region_consumes_a_leading_trailing_blank_before_non_newline_content() {
        let text = "before\n# >>> anvil-managed: x\nbody\n# <<< anvil-managed: x\n\nafter";
        assert_eq!(remove_region(text, "x", SYN).unwrap(), "before\nafter");
    }
