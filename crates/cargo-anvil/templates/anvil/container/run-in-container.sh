...
# discovery runs. Worktrees created from Linux or WSL already record a path
# this Git can resolve, and an ordinary repository has a `.git` directory.
if [[ -f .git ]]; then
    linked_git_dir="$(sed -n 's/^gitdir: //p' .git | tr -d '\r')"
    case "$linked_git_dir" in
        [A-Za-z]:[\\/]*)
            if ! command -v wslpath >/dev/null 2>&1; then
                echo 'anvil-container: wslpath is required to run a Windows-created linked worktree. Run the Windows driver from PowerShell, or recreate the worktree from Linux.' >&2
                exit 1
            fi
            if ! linked_git_dir="$(wslpath -a "$linked_git_dir" 2>/dev/null)" || [[ ! -d "$linked_git_dir" ]]; then
                echo 'anvil-container: could not resolve the Windows linked-worktree gitdir pointer in WSL. Verify that the worktree metadata still exists.' >&2
                exit 1
            fi
            export GIT_DIR="$linked_git_dir"
...
if ! git_dir="$(git rev-parse --absolute-git-dir 2>/dev/null)" ||
    ! git_common_dir="$(git rev-parse --path-format=absolute --git-common-dir 2>/dev/null)"; then
    echo 'anvil-container: could not resolve the Git metadata directories. Git 2.31 or newer is required.' >&2
    exit 1
fi
linked_worktree_args=()
if [[ "$git_dir" != "$git_common_dir" ]]; then
    case "$git_dir" in
        "$git_common_dir"/*) git_dir_relative="${git_dir#"$git_common_dir"/}" ;;
        *)
            echo 'anvil-container: the linked-worktree Git directory is outside its common Git directory; mounting it would expose an unrelated host path.' >&2
            exit 1
            ;;
    esac
    linked_worktree_args=(
        --mount "type=bind,source=$git_common_dir,target=/anvil-git,readonly"
        --env "GIT_DIR=/anvil-git/$git_dir_relative"
...
run_args=(
    ...
    "${mount_args[@]}"
    ${linked_worktree_args[@]+"${linked_worktree_args[@]}"}
    --workdir /workspace
)
