...
//!
//! [`stated::check`](super::super::stated::check) and [`indexes`](super::indexes::indexes) each
//! drive their own [`syn::visit::Visit`] over the same file, and the [`Collector`](super::Collector)
//! that follows them drives a third. The two pre-passes visit completely disjoint sets of node
//! kinds — `Audit` reads attributes and function-like items, `Walk` reads structs, `use`s,
//! constants and a handful of numeric-looking expressions — so nothing about combining them into
//! one walk changes what either one sees or in what order it sees it: every visit method below is
//! exactly the local update the corresponding standalone type already made, run in the same
//! recursive descent, just not paying for that descent twice.
//!
//! [`super::defaults::DefaultPaths`] deliberately stays out of this fusion. It never was a
//! recursive [`syn::visit::Visit`] walk — [`DefaultPaths::of`](super::defaults::DefaultPaths::of)
//! is a single pass over `file.items` — so folding it in here would not remove a traversal, only
//! move an already-cheap one.

use syn::visit::{self, Visit};
use syn::{
    Attribute, ExprBinary, ExprForLoop, ExprIndex, ExprMethodCall, ImplItemConst, ImplItemFn, ItemConst, ItemFn, ItemStatic, ItemStruct,
    ItemUse, TraitItemConst, TraitItemFn,
};
...
pub(in crate::ops::collect) fn run(file: &SourceFile, selection: &Selection) -> Result<Indexes> {
    let mut combined = PhaseOne {
        audit: Audit::default(),
        walk: Walk::new(selection),
    };
