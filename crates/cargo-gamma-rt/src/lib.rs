...
#![cfg_attr(
    ...
    expect(
        dead_code,
        reason = "Miri cannot execute the platform initializer, so the runtime entry-point graph is intentionally unreachable"
    )
)]
...
//! # Selection protocol
//!
//! The active mutant is named by the [`ACTIVE_VAR`] environment variable, captured exactly once
//! during process startup before user code can start threads. The value is a decimal mutant
//! ordinal. [`NONE`] means no mutant is active, which is how the baseline run and every ordinary
//! build behave — including builds of proc macros, where an active mutant could otherwise hang the
//! compiler.
//!
//! An unset, empty, or unparsable value all mean [`NONE`]. A build that links this crate but is not
...
//!
//! [`Either`] is the one other thing instrumented source mentions, and it exists because the guard
...
//! ```
//!
//! See [`Either`] for why this is not a `Box<dyn Iterator>`.
//!
