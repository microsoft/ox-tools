...
6. **Plain-cargo fallback**: a developer with only `cargo` installed (no `just`, no
   `cargo-ox-check-update`) can still build and run tests.
...
> **`cargo-ox-check-update` writes files. `just` runs them. The repo composes everything.**
...
`cargo ox-check-update`). The PR tier is made up of a small set of *check groups* — each group is a
...
A user with only `just` installed (no `cargo-ox-check-update`) can run any check, any group, or any tier
without ever invoking the tool. `cargo-ox-check-update` is only required by the maintainer who wants to
...
- **owned** — the tool fully writes the file. There is no in-file checksum line; ox-check
  ...
  (`.ox-check.lock`). An advisory one-line `# Managed by cargo-ox-check-update` comment may appear
