...
This document defines how `cargo-ox-check-update` is kept correct over time. The headline mechanism
is dogfooding — the `microsoft/ox-tools` repo, where cargo-ox-check-update itself lives, uses
...
`microsoft/ox-tools` is the canonical adopter of `cargo-ox-check-update`. Its `.github/workflows/`,
...
1. `cargo build --locked -p cargo-ox-check-update` — build the binary from source.
2. `target/debug/cargo-ox-check-update ox-check-update` — regenerate every owned file and managed region.
...
Under `crates/cargo-ox-check-update/tests/snapshots.rs`, three integration tests drive the full
...
stable across version bumps (the manifest carries `rendered_by = "cargo-ox-check-update <ver>"`
...
`crates/cargo-ox-check-update/tests/schemas.rs` covers the subset that can be
...
jobs:
  regenerate-check:
    ...
    steps:
      ...
      - run: cargo build --locked -p cargo-ox-check-update
      - name: Regenerate emitted files
        run: ./target/debug/cargo-ox-check-update ox-check-update
      - name: Assert no drift
        run: |
          if ! git diff --exit-code; then
            echo "::error::cargo-ox-check-update changed files. Run 'cargo ox-check-update' locally and commit the diff."
            exit 1
          fi
...
The very first cargo-ox-check-update PR cannot dogfood itself — the binary doesn't exist yet.
Bootstrap plan:
...
### Breaking changes inside cargo-ox-check-update
...
breakage is **not stuck** — every PR builds cargo-ox-check-update from source on its own branch,
...
ox-tools never depends on the published crates.io version of cargo-ox-check-update for its own
...
- **Long-lived divergence** — a repo that's been on an old cargo-ox-check-update version for
...
| Path                                                | Purpose                                                                 |
|-----------------------------------------------------|-------------------------------------------------------------------------|
| `crates/cargo-ox-check-update/tests/snapshots.rs`             | Snapshot tests over the three backend combinations (insta).             |
| `crates/cargo-ox-check-update/tests/snapshots/`               | Committed snapshot files (one per backend combination).                 |
| `crates/cargo-ox-check-update/tests/fixtures/`                | Integration test fixtures (one directory per shape).                    |
| `crates/cargo-ox-check-update/tests/update.rs`                | Per-fixture assertions for opt-outs, customizations, migrations, single-crate. |
| `crates/cargo-ox-check-update/tests/schemas.rs`               | actionlint / taplo / just-parse wrappers run against the emitted output. |
...
- A dedicated `cargo ox-check-update verify` subcommand that runs `update --dry-run` + all
  schema validators + the manifest-consistency check in one step, for local
  pre-commit use.
- A small set of "downstream canary" repos in `microsoft/` org that pin cargo-ox-check-update
