...
use cargo_ox_check_update::cli::Cli;
use cargo_ox_check_update::decision::Decision;
use cargo_ox_check_update::plan::Target;
use cargo_ox_check_update::run::{RunOutcome, run_update};
...
fn single_crate_emits_crate_lints_and_justfiles() {
    ...
    assert!(
        cargo.contains("ox-check-managed: ox-check-lints"),
        "single-crate fixture should receive the per-crate lints region; got:\n{cargo}"
    );
    assert!(
        !cargo.contains("ox-check-workspace-lints"),
...
fn empty_region_is_treated_as_opt_out() {
    use cargo_ox_check_update::emit::shared_configs::RUSTFMT_REGION_ID;
    use cargo_ox_check_update::region::{CommentSyntax, upsert_region};
...
/// get ox-check's regions spliced in without losing any user content.
#[test]
fn migration_preserves_user_content() {
    ...
    assert!(
        justfile.contains("ox-check-imports"),
        "ox-check imports region must be spliced into the existing Justfile"
    );
    ...
    assert!(
        cargo.contains("ox-check-workspace-lints"),
        "ox-check workspace lints region must be spliced into Cargo.toml"
    );
    ...
    assert!(
        deny.contains("ox-check-deny"),
        "ox-check deny region must be spliced into deny.toml"
    );
