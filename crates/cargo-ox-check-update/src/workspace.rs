...
//!
//! `cargo-ox-check-update` operates from the *workspace root*: the directory whose
...
pub fn find_workspace_root(start: &Path) -> Result<PathBuf, AppError> {
    ...
    nearest.ok_or_else(|| {
        app_err!(
            "no Cargo.toml found at or above '{}'; cargo-ox-check-update must be run inside a Cargo workspace",
            start.display()
        )
    })
}
