...
#[cfg(unix)]
impl<R> InterruptiblePipe<R>
where
    R: std::os::fd::AsRawFd,
{
    /// Switches `inner` to nonblocking reads.
    ///
    /// # Errors
    ///
    /// Returns the operating system's error when the descriptor flags cannot
    /// be read or updated.
    pub fn new(inner: R) -> io::Result<Self> {
        let descriptor = inner.as_raw_fd();

        // SAFETY: `F_GETFL` reads flags for the borrowed live descriptor and
        ...
        if flags & libc::O_NONBLOCK == 0 {
            // SAFETY: `F_SETFL` updates the status flags of the same borrowed
            // live descriptor. Preserving every existing flag avoids changing
            // any property other than nonblocking I/O.
...
mod tests {
    ...
    fn child_pipe_reads_are_pending_data_then_eof() {
        ...
        let mut pipe = InterruptiblePipe::new(stdout).expect("make child pipe interruptible");
    ...
    R: Read + std::os::windows::io::AsRawHandle,
{
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let ready = unsafe {
            PeekNamedPipe(
                self.inner.as_raw_handle().cast::<core::ffi::c_void>() as HANDLE,
                core::ptr::null_mut(),
                0,
                core::ptr::null_mut(),
...
impl<R> InterruptiblePipe<R> {
    /// Rejects capture where no bounded child-pipe readiness primitive exists.
    pub fn new(inner: R) -> io::Result<Self> {
        drop(inner);
        Err(io::Error::new(
