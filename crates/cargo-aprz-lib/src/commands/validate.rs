...
"#,
    ...
    fn test_medium_risk_threshold_below_range() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("medium_risk_threshold must be between 0 and 100")
        );
    }
    ...
    fn test_low_risk_threshold_below_range() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("low_risk_threshold must be between 0 and 100")
        );
    }
    ...
    fn test_medium_threshold_not_less_than_low_threshold() {
        ...
        std::fs::write(&config_path, "medium_risk_threshold = 80.0\nlow_risk_threshold = 50.0\n").expect("Failed to write test config");
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("medium_risk_threshold (80) must be less than low_risk_threshold (50)")
        );
    }
    ...
    fn test_equal_thresholds_rejected() {
        ...
        std::fs::write(&config_path, "medium_risk_threshold = 50.0\nlow_risk_threshold = 50.0\n").expect("Failed to write test config");
    ...
    fn test_valid_custom_thresholds() {
        ...
        std::fs::write(&config_path, "medium_risk_threshold = 25.0\nlow_risk_threshold = 75.0\n").expect("Failed to write test config");
