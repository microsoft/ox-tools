...
fn build_cel_context(metrics: impl IntoIterator<Item: core::borrow::Borrow<Metric>>, now: DateTime<Local>) -> Context<'static> {
    ...
    for metric in metrics {
        ...
        if let Some((prefix, suffix)) = name.split_once('.') {
            let _ = root_map.entry(prefix).or_default().insert(metric_key(name, suffix), cel_value);
        } else {
...
mod tests {
    ...
    fn test_evaluation_outcome_creation() {
        ...
        let denied = Appraisal::new(
            Risk::High,
            vec![ExpressionOutcome::new("r".into(), "reason".into(), ExpressionDisposition::False)],
            1,
            0,
            0.0,
        );
    ...
    fn test_points_failed_expression_counts_as_unawarded_configured_points() {
        ...
        assert_eq!(outcome.risk, Risk::Medium);
        assert!(matches!(
            outcome.expression_outcomes[1].disposition,
            ExpressionDisposition::Failed(_)
        ));
    }
    ...
    fn test_mostly_inconclusive_policy_cannot_report_a_perfect_score() {
        let mut expressions = vec![Expression::new("passing", None, "stars > 100", Some(10)).unwrap()];
        for index in 0..9 {
        ...
        let outcome = evaluate(&[], &expressions, &metrics, test_timestamp(), MEDIUM_THRESHOLD, LOW_THRESHOLD);
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn test_zero_weight_success_does_not_make_positive_failures_conclusive() {
        let zero_weight_success = Expression::new("zero", None, "stars > 100", Some(0)).unwrap();
        let positive_failure = Expression::new("failed", None, "undefined > 0", Some(5)).unwrap();
        let metrics = vec![Metric::with_value(&STARS_DEF, MetricValue::UInt(150))];

        let outcome = evaluate(
            &[],
            &[zero_weight_success, positive_failure],
            &metrics,
            test_timestamp(),
            MEDIUM_THRESHOLD,
            LOW_THRESHOLD,
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn test_zero_weight_false_does_not_make_positive_failures_conclusive() {
        let zero_weight_false = Expression::new("zero", None, "stars > 200", Some(0)).unwrap();
        let positive_failure = Expression::new("failed", None, "undefined > 0", Some(5)).unwrap();
        let metrics = vec![Metric::with_value(&STARS_DEF, MetricValue::UInt(150))];

        let outcome = evaluate(
            &[],
            &[zero_weight_false, positive_failure],
            &metrics,
            test_timestamp(),
            MEDIUM_THRESHOLD,
            LOW_THRESHOLD,
    ...
    #[test]
    #[cfg_attr(miri, ignore)]
    fn test_positive_weight_false_check_makes_policy_conclusive() {
        let false_check = Expression::new("false_check", None, "stars > 200", Some(5)).unwrap();
        let failed_check = Expression::new("failed_check", None, "undefined > 0", Some(5)).unwrap();
        let metrics = vec![Metric::with_value(&STARS_DEF, MetricValue::UInt(150))];

        let outcome = evaluate(
            &[],
            &[false_check, failed_check],
            &metrics,
            test_timestamp(),
            MEDIUM_THRESHOLD,
            LOW_THRESHOLD,
        ...
        assert_eq!(outcome.point_totals(), Some((0, 10)));
        assert_eq!(outcome.weighted_score(), Some(0.0));
        assert_eq!(outcome.risk(), Risk::High);
        assert!(!outcome.is_weighted_evaluation_failure());
    }
