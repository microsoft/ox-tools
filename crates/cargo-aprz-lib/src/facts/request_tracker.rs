...
const fn blink_is_on(elapsed_ms: u128) -> bool {
...
mod tests {
    ...
    #[test]
    fn completing_the_boundary_request_marks_only_that_topic_done() {
        let tracker = test_tracker();
        tracker.add_requests(TrackedTopic::Coverage, 2);
        tracker.add_requests(TrackedTopic::Docs, 2);
        ...
        assert_eq!(
            tracker.counters[TrackedTopic::Docs.index()].status.load(Ordering::Acquire),
            TopicStatus::Active as u8
        );

        let (total, completed, message) = RequestTracker::progress_reporter_callback(&tracker.counters, true);
        assert_eq!(total, 4);
        assert_eq!(completed, 3);
        assert_eq!(strip_ansi(&message), "2/2 coverage, 1/2 docs");
        assert!(message.contains("\x1b[32m"), "completed topic should be green: {message:?}");
    }

    #[test]
    fn completing_without_issued_requests_does_not_mark_done() {
        let tracker = test_tracker();

        tracker.complete_request(TrackedTopic::Repos);

        assert_eq!(
            tracker.counters[TrackedTopic::Repos.index()].status.load(Ordering::Acquire),
            TopicStatus::Active as u8
        );
        let (total, completed, message) = RequestTracker::progress_reporter_callback(&tracker.counters, false);
        assert_eq!(total, 0);
    ...
    #[test]
    fn blink_phase_changes_only_at_half_second_boundaries() {
        assert!(blink_is_on(0));
        assert!(blink_is_on(499));
        assert!(!blink_is_on(500));
        assert!(!blink_is_on(999));
        assert!(blink_is_on(1000));
    }
