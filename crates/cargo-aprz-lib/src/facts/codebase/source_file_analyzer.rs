...
impl<'a> SourceFileAnalyzer<'a> {
    ...
    fn process_token(&mut self, token: &ra_ap_syntax::SyntaxToken) {
        ...
        if kind == SyntaxKind::UNSAFE_KW {
            ...
            let parent = token
                .parent()
                .expect("only the syntax tree's root node lacks a parent, and the root is never a token");
            match parent.kind() {
    ...
    fn is_test_context(node: &SyntaxNode) -> bool {
        match node.kind() {
            SyntaxKind::MODULE => {
                let module = ast::Module::cast(node.clone()).expect("the match arm already established that the node is a MODULE");
                return Self::has_test_attribute(&module);
            }
            SyntaxKind::FN => {
                let function = ast::Fn::cast(node.clone()).expect("the match arm already established that the node is a FN");
...
"#;
    ...
    #[test]
    #[cfg_attr(miri, ignore = "Miri detects UB in external rowan crate")]
    fn test_cfg_test_module_context_does_not_leak_to_following_items() {
        let source = r"
#[test]
mod tests {
    fn helper_only_marked_by_module() {
        assert!(true);
    }
}
...
";
        ...
        let helper_line = source
            .lines()
            .position(|line| line.contains("helper_only_marked_by_module"))
            .expect("fixture contains helper function");
        let production_line = source
            .lines()
            .position(|line| line.contains("production_after_tests"))
            .expect("fixture contains production function");

        assert!(
            analyzer.line_flags[helper_line] & LINE_TEST != 0,
            "a module marked by a test attribute must make its body test code"
        );
        assert!(
            analyzer.line_flags[production_line] & LINE_TEST == 0,
            "items after a cfg(test) module must return to production context"
        );
    }
...
";
    ...
    #[test]
    #[cfg_attr(miri, ignore = "Miri detects UB in external rowan crate")]
    fn test_malformed_test_attribute_with_arguments_is_production_code() {
        let source = "
#[test(custom)]
fn production_because_test_attribute_has_arguments() {
    assert_eq!(2 + 2, 4);
}
";
        let stats = analyze_source_file(source);
...
";
    ...
    fn test_cfg_predicate_is_test_only() {
        ...
        assert!(cfg_predicate_is_test_only("any(all(feature = \"x\", test), test)"));
...
";
    ...
    #[test]
    #[cfg_attr(miri, ignore = "Miri detects UB in external rowan crate")]
    fn test_unsafe_impl_and_trait_are_counted() {
        let source = r#"
unsafe trait Marker {}
...
"#;
        ...
        // The `unsafe trait` and `unsafe impl` keywords count; the one attached
        // to the extern block does not.
        assert_eq!(stats.unsafe_count, 2);
    }

    #[test]
    #[cfg_attr(miri, ignore = "Miri detects UB in external rowan crate")]
    fn test_nested_unsafe_blocks_are_counted_separately() {
        let source = r"
pub fn outer(p: *const u32) -> u32 {
    unsafe {
        let inner = unsafe { *p };
        inner
    }
