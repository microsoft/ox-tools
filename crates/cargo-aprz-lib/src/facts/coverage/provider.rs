...
impl Provider {
    ...
    async fn fetch_coverage_data_for_repo_core(&self, repo_spec: &RepoSpec) -> ProviderResult<CoverageData> {
        ...
        let coverage_data = CoverageData { code_coverage_percentage };
    ...
    async fn try_branch_coverage(&self, repo_spec: &RepoSpec, service: &str, branch: &str) -> Result<Option<f64>> {
            ...
            && let Some(percent_str) = captures.get(1)
        {
            ...
            // PERCENT_REGEX's capture group only matches a well-formed decimal literal, so this
            // parse cannot fail (even an absurdly long digit run parses to infinity, which is Ok).
            let coverage = percent_str
                .parse::<f64>()
                .expect("PERCENT_REGEX capture group only ever matches a well-formed decimal literal");
