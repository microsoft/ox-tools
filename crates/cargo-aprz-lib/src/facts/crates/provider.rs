...
use super::tables::{
    CategoriesTableIndex, CategoryId, CrateId, CratesTableIndex, KeywordId, KeywordsTableIndex, OwnerKind as TableOwnerKind, Table,
    TableMgr, TeamId, TeamsTableIndex, UserId, UsersTableIndex, VersionId, VersionsTableIndex,
};
        ...
        for (lean_row, index) in self.table_mgr.versions_table().iter_lean() {
            ...
            if let Some(data) = crate_data.get_mut(&lean_row.crate_id) {
                ...
                let requested_version = (remaining_versions != 0)
                    .then(|| needed_versions.get(&lean_row.crate_id))
                    .flatten()
                    .and_then(|versions| versions.get(&row.num));
                if let Some(crate_ref) = requested_version {
            ...
            if let Some(&crate_id) = all_version_to_crate.get(&row.version_id) {
                ...
                *crate_monthly.entry(crate_id).or_default().entry(month_key).or_insert(0) += row.downloads;
                ...
                if version_ids.contains(&row.version_id) {
                    *version_monthly.entry(row.version_id).or_default().entry(month_key).or_insert(0) += row.downloads;
                }
            }
        }
...
fn monthly_btree_to_vec<K: Eq + core::hash::Hash>(monthly: HashMap<K, BTreeMap<(i32, u32), u64>>) -> HashMap<K, Vec<(NaiveDate, u64)>> {
    monthly
        .into_iter()
        .map(|(key, month_map)| {
            let vec = month_map
...
mod tests {
    ...
    fn csv(headers: &[&str], rows: Vec<Vec<String>>) -> String {
        ...
        let bytes = writer.into_inner().expect("flushing an in-memory CSV must not fail");
        String::from_utf8(bytes).expect("CSV data written by the test is valid UTF-8")
    }
    ...
    fn crate_row(id: u64, name: &str, now: DateTime<Utc>) -> Vec<String> {
        vec![
            id.to_string(),
            name.to_owned(),
            timestamp(now - Duration::days(400)),
            timestamp(now),
            format!("the {name} crate"),
            String::new(),
            String::new(),
            String::new(),
            String::new(),
    ...
    fn version_row(fields: VersionRowFields<'_>, now: DateTime<Utc>) -> Vec<String> {
        let created_at = now - Duration::days(fields.age_days);
        vec![
            fields.id.to_string(),
            fields.crate_id.to_string(),
            fields.num.to_owned(),
            timestamp(created_at),
            timestamp(created_at),
            fields.downloads.to_string(),
            "{\"default\":[\"std\"],\"std\":[]}".to_owned(),
            if fields.yanked { "t" } else { "f" }.to_owned(),
            "MIT".to_owned(),
            "12345".to_owned(),
            "abc123".to_owned(),
            String::new(),
    ...
    fn crates_table_csv(now: DateTime<Utc>) -> (&'static str, String) {
        ...
        rows.extend([
            ...
            crate_row(1002, "abcxefghij", now),
            crate_row(1003, "abcdxfghij", now),
            crate_row(1004, "abcdxfghiy", now),
            crate_row(1100, "abcdefghijklmnopqrsu", now),
            crate_row(1101, "xbcdefghijxlmnoxqrsx", now),
            crate_row(1102, "abcxefghxjklxnxpqrst", now),
            crate_row(1103, "xbcdefghijklmnopqrsx", now),
            crate_row(11, "offset-target", now),
        ]);
    ...
    fn versions_table_csv(now: DateTime<Utc>) -> (&'static str, String) {
        (
            "versions.csv",
            csv(
                &[
                ...
                vec![
                    version_row(version_fields(100, 10, "0.1.0", "first row", 111, 400, false), now),
                    version_row(version_fields(101, 10, "0.2.0", "second row", 222, 300, false), now),
                    version_row(version_fields(102, 10, "0.3.0", "third row", 333, 200, false), now),
                    version_row(version_fields(300, 30, "0.1.0", "first dependent row", 10, 10, false), now),
                    version_row(version_fields(301, 30, "0.1.1", "second dependent row", 11, 10, false), now),
                    version_row(version_fields(399, 30, "0.1.3", "late dependent row", 13, 10, false), now),
                    version_row(version_fields(301, 40, "0.1.2", "duplicate dependent row", 12, 10, false), now),
                    version_row(version_fields(103, 10, "1.2.3", "fourth-row exact target", 444, 22, false), now),
                    version_row(version_fields(104, 10, "1.2.3", "duplicate exact target", 999, 21, false), now),
                    version_row(version_fields(200, 20, "9.0.0-alpha.1", "pre-release row", 555, 20, false), now),
                    version_row(version_fields(201, 20, "8.0.0", "yanked row", 666, 19, true), now),
    ...
    fn version_downloads_csv(now: DateTime<Utc>) -> (&'static str, String) {
        (
    ...
    fn dependencies_csv() -> (&'static str, String) {
        (
            "dependencies.csv",
            csv(
                &[
    ...
    fn crate_downloads_csv() -> (&'static str, String) {
        (
            "crate_downloads.csv",
            csv(
                &["crate_id", "downloads"],
                vec![
                    vec!["10".to_owned(), "1000".to_owned()],
                    vec!["20".to_owned(), "2000".to_owned()],
                    vec!["10".to_owned(), "9999".to_owned()],
                ],
            ),
        )
    }

    fn provider_dump(now: DateTime<Utc>) -> Vec<u8> {
        tar_gz(&[
            crates_table_csv(now),
            versions_table_csv(now),
    ...
    async fn provider_from_dump(now: DateTime<Utc>) -> Provider {
        let server = MockServer::start().await;
        Mock::given(method("GET"))
            .and(path("/db-dump.tar.gz"))
            .respond_with(ResponseTemplate::new(200).set_body_bytes(provider_dump(now)))
            .mount(&server)
            .await;

        let cache = tempfile::tempdir().expect("creating a temporary cache directory");
        Provider::new(
            cache.path(),
    ...
    fn crate_ref(name: &str, version: Option<&str>) -> CrateRef {
        CrateRef::new(
            name,
            version.map(|value| Version::parse(value).expect("test versions must be valid semver")),
        )
    }
    ...
    #[tokio::test]
    #[cfg_attr(miri, ignore = "Miri cannot memory-map files or run a mock HTTP server")]
    async fn exact_version_lookup_reads_the_matched_table_row() {
        let now = "2026-08-11T04:13:36Z"
            .parse::<DateTime<Utc>>()
            .expect("the fixed test timestamp is valid");
        let provider = provider_from_dump(now).await;
        ...
        let (spec, result) = results.pop().expect("the provider returns one result per request");
        assert_eq!(spec.version().to_string(), "1.2.3");

        let ProviderResult::Found(data) = result else {
            panic!("expected the exact version to be found");
        };
    ...
    #[tokio::test]
    #[cfg_attr(miri, ignore = "Miri cannot memory-map files or run a mock HTTP server")]
    async fn latest_version_lookup_uses_the_preferred_stable_row() {
        let now = "2026-08-11T04:13:36Z"
            .parse::<DateTime<Utc>>()
            .expect("the fixed test timestamp is valid");
        let provider = provider_from_dump(now).await;
        let requested = [crate_ref("latest-target", None)];
        let mut results: Vec<_> = provider.get_crates_data(&requested, &NoOpProgress, false).await.collect();

        assert_eq!(results.len(), 1);
        let (spec, result) = results.pop().expect("the provider returns one result per request");
        assert_eq!(spec.version().to_string(), "2.0.0");
    ...
    #[tokio::test]
    #[cfg_attr(miri, ignore = "Miri cannot memory-map files or run a mock HTTP server")]
    async fn exact_version_lookup_stops_before_later_duplicate_rows() {
        let now = "2026-08-11T04:13:36Z"
            .parse::<DateTime<Utc>>()
            .expect("the fixed test timestamp is valid");
        let provider = provider_from_dump(now).await;
        ...
        assert_eq!(results.len(), 1);
        let (_spec, result) = results.pop().expect("the provider returns one result per request");
    ...
    #[tokio::test]
    #[cfg_attr(miri, ignore = "Miri cannot memory-map files or run a mock HTTP server")]
    async fn exact_version_lookup_is_not_overwritten_during_a_mixed_latest_query() {
        let now = "2026-08-11T04:13:36Z"
            .parse::<DateTime<Utc>>()
            .expect("the fixed test timestamp is valid");
        let provider = provider_from_dump(now).await;
        let requested = [crate_ref("offset-target", Some("1.2.3")), crate_ref("latest-target", None)];
        let results: Vec<_> = provider.get_crates_data(&requested, &NoOpProgress, false).await.collect();
        assert_eq!(results.len(), 2);
        assert_eq!(
            results.iter().map(|(spec, _)| spec.name()).collect::<Vec<_>>(),
            ["offset-target", "latest-target"]
        );
        let data = results
            .iter()
            .find_map(|(spec, result)| (spec.name() == "offset-target").then_some(result))
            .expect("the mixed query includes offset-target");

        let ProviderResult::Found(data) = data else {
            panic!("expected the exact version to be found");
        };
        assert_eq!(data.version_data.description, "fourth-row exact target");
        assert_eq!(data.version_data.downloads, 444);
    ...
    #[tokio::test]
    #[cfg_attr(miri, ignore = "Miri cannot memory-map files or run a mock HTTP server")]
    async fn suggestions_keep_the_first_three_equal_best_matches() {
        let now = "2026-08-11T04:13:36Z"
            .parse::<DateTime<Utc>>()
            .expect("the fixed test timestamp is valid");
        let provider = provider_from_dump(now).await;
        ...
        assert_eq!(
            suggestions.as_ref(),
            [
                CompactString::from("abcdefghik"),
                CompactString::from("abcdefxhij"),
    ...
    #[tokio::test]
    #[cfg_attr(miri, ignore = "Miri cannot memory-map files or run a mock HTTP server")]
    async fn suggestions_replace_the_lowest_scoring_full_slot() {
        let now = "2026-08-11T04:13:36Z"
            .parse::<DateTime<Utc>>()
            .expect("the fixed test timestamp is valid");
        let provider = provider_from_dump(now).await;
        let requested = [crate_ref("abcdefghijklmnopqrst", None)];
        let mut results: Vec<_> = provider.get_crates_data(&requested, &NoOpProgress, true).await.collect();
