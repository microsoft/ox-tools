...
mod tests {
    ...
    #[test]
    #[cfg_attr(miri, ignore = "Miri cannot call GetTempPathW")]
    fn load_entry_just_before_ttl_boundary_is_fresh() {
        let tmp = tempfile::tempdir().unwrap();
        let ttl_seconds = 3600i64;
        let fresh_time = Utc::now() - chrono::Duration::seconds(ttl_seconds - 5);
        ...
        match cache.load::<TestData>("fresh.bin") {
            CacheResult::Data(data) => assert_eq!(data.value, 5),
            other => panic!("expected fresh Data before the TTL boundary, got {other:?}"),
        }
    }
    ...
    fn zero_ttl_entry_created_this_second_is_expired() {
        ...
        let path = tmp.path().join("immediate.bin");
        let file = File::create(&path).unwrap();
        rmp_serde::encode::write(&mut BufWriter::new(file), &envelope).unwrap();

        let cache = make_cache(tmp.path(), 0);
        assert!(matches!(cache.load::<TestData>("immediate.bin"), CacheResult::Miss));
    }
    ...
    fn saving_fails_when_the_cache_directory_cannot_be_created() {
        let tmp = tempfile::tempdir().unwrap();
        // A regular file where the cache directory belongs makes `create_dir_all` fail.
        let blocker = tmp.path().join("blocker");
        fs::write(&blocker, b"not a directory").unwrap();

        let cache = make_cache(&blocker.join("nested"), 3600);

        let data = TestData {
            name: "test".to_string(),
            value: 1,
        };
        let err = cache.save("item.bin", &data).unwrap_err();
        assert!(err.to_string().contains("creating directory"), "{err}");

        let err = cache.save_no_data("item.bin", "unavailable").unwrap_err();
        assert!(err.to_string().contains("creating directory"), "{err}");
    }

    #[test]
    #[cfg_attr(miri, ignore = "Miri cannot call GetTempPathW")]
    fn dir_returns_the_configured_directory() {
        let tmp = tempfile::tempdir().unwrap();
        let cache = make_cache(tmp.path(), 3600);
    ...
    fn saving_to_a_path_with_no_parent_skips_directory_creation() {
        ...
        // on disk, so the write fails at the file itself rather than at the directory.
        let cache = make_cache(Path::new(""), 3600);

        let data = TestData {
            name: "test".to_string(),
            value: 7,
        };
        let err = cache.save("", &data).unwrap_err();
        assert!(err.to_string().contains("creating cache file"), "{err}");
    }
}
