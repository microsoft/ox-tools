...
use cargo_aprz_lib::internals::facts::docs::{DocsData, DocsMetrics, Provider};
use cargo_aprz_lib::internals::facts::{CrateSpec, Progress, ProviderResult, RequestTracker};
use semver::Version;
...
async fn fetch_one(provider: &Provider, crate_spec: CrateSpec) -> ProviderResult<DocsData> {
    let progress = Arc::new(NoOpProgress) as Arc<dyn Progress>;
    let tracker = RequestTracker::new(&progress);
    let results: Vec<_> = provider.get_docs_data(vec![crate_spec].into(), &tracker).await.collect();
    assert_eq!(results.len(), 1);
...
fn spec(name: &str, version: &str) -> CrateSpec {
    CrateSpec::from_arcs(
        Arc::from(name),
        Arc::new(Version::parse(version).expect("test version is well-formed")),
    )
}

#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_docs_provider_undecodable_payload_is_unavailable_and_cached() {
    let mock_server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/crate/broken/1.0.0/json"))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(b"this is not zstd".to_vec()))
        .mount(&mock_server)
        .await;
    ...
    let first = fetch_one(&provider, spec("broken", "1.0.0")).await;
    ...
    let second = fetch_one(&provider, spec("broken", "1.0.0")).await;
...
#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_docs_provider_reports_an_error_for_an_unexpected_status() {
    let mock_server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/crate/badrequest/1.0.0/json"))
    ...
    let result = fetch_one(&provider, spec("badrequest", "1.0.0")).await;
    match result {
        ProviderResult::Error(e) => assert!(format!("{e:#}").contains("badrequest"), "unexpected error: {e:#}"),
...
#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_docs_provider_reports_an_error_when_caching_fails() {
    if !fixture_exists() {
        eprintln!("Skipping test: fixture file {FIXTURE_PATH} not found");
        return;
    }
    ...
    Mock::given(method("GET"))
        .and(path("/crate/anyhow/1.0.100/json"))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(zst_data))
        .mount(&mock_server)
        .await;

    let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
    // A file where the cache directory belongs makes every cache write fail.
    let blocker = temp_dir.path().join("blocker");
    fs::write(&blocker, b"not a directory").expect("write blocker file");

    let cache = Cache::new(&blocker, core::time::Duration::MAX, false);
    let provider = Provider::new(cache, Some(&mock_server.uri()));

    let result = fetch_one(&provider, spec("anyhow", "1.0.100")).await;
    assert!(matches!(result, ProviderResult::Error(_)), "got {result:?}");
}

#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_docs_provider_survives_a_failing_negative_cache_write() {
    let mock_server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/crate/missing/1.0.0/json"))
        .respond_with(ResponseTemplate::new(404))
        .mount(&mock_server)
        .await;
    Mock::given(method("GET"))
    ...
    // A 404 and an undecodable payload both still report Unavailable even though the
    // negative cache entry cannot be written.
    let not_found = fetch_one(&provider, spec("missing", "1.0.0")).await;
    assert!(matches!(not_found, ProviderResult::Unavailable(_)), "got {not_found:?}");

    let undecodable = fetch_one(&provider, spec("broken", "1.0.0")).await;
    assert!(matches!(undecodable, ProviderResult::Unavailable(_)), "got {undecodable:?}");
}
