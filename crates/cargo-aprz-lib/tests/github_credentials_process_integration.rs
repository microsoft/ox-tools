...
impl EnvironmentGuard {
    fn set(values: impl IntoIterator<Item = (&'static str, OsString)>) -> Self {
        let mut previous = Vec::new();
        for (name, value) in values {
            previous.push((OsString::from(name), std::env::var_os(name)));
            // SAFETY: this integration-test binary contains one current-thread
            // test, and every modified value is restored by Drop.
            unsafe {
    ...
    fn drop(&mut self) {
        for (name, value) in self.previous.drain(..).rev() {
            if let Some(value) = value {
                // SAFETY: the single current-thread test is still the only code
...
async fn run_discovers_an_enterprise_token_through_the_production_process_path() {
    ...
    let mut environment = vec![
        ...
        ("GITHUB_TOKEN", OsString::from(" \t ")),
        ("FAKE_GH_LOG", gh_log.clone().into_os_string()),
    ];
    if cfg!(windows) {
        environment.push(("PATHEXT", OsString::from(".COM;.EXE;.BAT;.CMD")));
    }
    let _environment = EnvironmentGuard::set(environment);

    let github_url = github.uri();
    let github_hostname = Url::parse(&github_url)
    ...
    assert!(
        gh_call.contains(&format!("args=auth|token|--hostname|{github_hostname}")),
