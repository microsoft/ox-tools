...
use cargo_aprz_lib::internals::facts::coverage::{CoverageData, Provider};
use cargo_aprz_lib::internals::facts::{CrateSpec, Progress, ProviderResult, RepoSpec, RequestTracker};
use semver::Version;
use url::Url;
...
/// Builds a crate spec pointing at the given repository URL.
fn crate_spec_for(repo_url: &str) -> CrateSpec {
    let url = Url::parse(repo_url).expect("test URL is well-formed");
    let repo_spec = RepoSpec::parse(&url).expect("test URL names a repository");
...
/// Runs the provider once for a single crate and returns the result.
async fn fetch_one(provider: &Provider, crate_spec: CrateSpec) -> ProviderResult<CoverageData> {
    let progress = Arc::new(NoOpProgress) as Arc<dyn Progress>;
    let tracker = RequestTracker::new(&progress);
    let results: Vec<_> = provider.get_coverage_data(vec![crate_spec].into(), &tracker).await.collect();
    assert_eq!(results.len(), 1);
    results.into_iter().next().expect("just asserted there is exactly one result").1
}

#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_coverage_provider_skips_unsupported_forge_and_caches_the_answer() {
    let mock_server = MockServer::start().await;
    let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
    let cache = Cache::new(temp_dir.path(), core::time::Duration::from_hours(8760), false);
    let provider = Provider::new(cache, Some(&mock_server.uri()));

    let crate_spec = crate_spec_for("https://codeberg.org/owner/repo");

    let first = fetch_one(&provider, crate_spec.clone()).await;
    assert!(matches!(first, ProviderResult::Unavailable(_)), "got {first:?}");

    // The second call is served from the negative cache entry written by the first.
    let second = fetch_one(&provider, crate_spec).await;
    assert!(matches!(second, ProviderResult::Unavailable(_)), "got {second:?}");

    let requests = mock_server.received_requests().await.unwrap();
    assert!(requests.is_empty(), "codecov does not host this forge, so it must not be queried");
}

#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_coverage_provider_handles_badge_without_a_percentage() {
    let mock_server = MockServer::start().await;
    for branch in ["main", "master"] {
        Mock::given(method("GET"))
            .and(path(format!("/gh/test/nopercent/branch/{branch}/graph/badge.svg")))
            .respond_with(ResponseTemplate::new(200).set_body_string("<svg><text>no numbers here</text></svg>"))
            .mount(&mock_server)
            .await;
    }
    ...
    let result = fetch_one(&provider, crate_spec_for("https://github.com/test/nopercent")).await;
    assert!(matches!(result, ProviderResult::Unavailable(_)), "got {result:?}");
}

#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_coverage_provider_falls_back_to_master_branch() {
    let mock_server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/gh/test/masteronly/branch/main/graph/badge.svg"))
        .respond_with(ResponseTemplate::new(404))
        .mount(&mock_server)
        .await;
    Mock::given(method("GET"))
        .and(path("/gh/test/masteronly/branch/master/graph/badge.svg"))
        .respond_with(ResponseTemplate::new(200).set_body_string("<svg><text>77.5%</text></svg>"))
        .mount(&mock_server)
        .await;
    ...
    match fetch_one(&provider, crate_spec_for("https://github.com/test/masteronly")).await {
        ProviderResult::Found(data) => assert!((data.code_coverage_percentage - 77.5).abs() < f64::EPSILON),
        other => panic!("Expected Found, got {other:?}"),
    }
}
...
#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_coverage_provider_reports_an_error_when_caching_fails() {
    let mock_server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/gh/test/repo/branch/main/graph/badge.svg"))
        .respond_with(ResponseTemplate::new(200).set_body_string("<svg><text>64%</text></svg>"))
        .mount(&mock_server)
        .await;

    let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
    // Cache entries live under "<host>/<owner>/<repo>.bin"; a file where the host directory
    // belongs makes every cache write fail.
    fs::write(temp_dir.path().join("github.com"), b"not a directory").expect("write blocker file");

    let cache = Cache::new(temp_dir.path(), core::time::Duration::from_hours(8760), false);
    let provider = Provider::new(cache, Some(&mock_server.uri()));

    let result = fetch_one(&provider, crate_spec_for("https://github.com/test/repo")).await;
    assert!(matches!(result, ProviderResult::Error(_)), "got {result:?}");
}

#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_coverage_provider_reports_an_error_when_caching_a_negative_result_fails() {
    let mock_server = MockServer::start().await;
    let temp_dir = tempfile::tempdir().expect("Failed to create temp dir");
    fs::write(temp_dir.path().join("codeberg.org"), b"not a directory").expect("write blocker file");

    let cache = Cache::new(temp_dir.path(), core::time::Duration::from_hours(8760), false);
    let provider = Provider::new(cache, Some(&mock_server.uri()));

    let result = fetch_one(&provider, crate_spec_for("https://codeberg.org/owner/repo")).await;
    assert!(matches!(result, ProviderResult::Error(_)), "got {result:?}");
}

#[tokio::test]
#[cfg_attr(miri, ignore = "Miri cannot call CreateIoCompletionPort")]
async fn test_coverage_provider_reports_an_error_for_persistent_server_failures() {
    let mock_server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/gh/test/broken/branch/main/graph/badge.svg"))
    ...
    let result = fetch_one(&provider, crate_spec_for("https://github.com/test/broken")).await;
    assert!(matches!(result, ProviderResult::Error(_)), "got {result:?}");
}
