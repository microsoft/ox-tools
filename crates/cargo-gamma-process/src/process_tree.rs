...
impl Drop for ReaperRunningGuard {
    fn drop(&mut self) {
        ...
        let restart = !reaper.retry_children.is_empty();
        CHILD_REAPER_READY.notify_all();
        drop(reaper);

        // A handoff can race an unwind after warning reporting released the
        // queue lock but before this guard cleared `running`. Restart here so
        // a child accepted during that window never waits for an unrelated
        // future caller to revive the queue.
        if restart {
            let _ignored = ensure_reaper();
        }
    }
}
...
mod tests {
    ...
    fn a_reaper_diagnostic_releases_the_lock_and_an_unwind_can_restart() {
        ...
        CHILD_REAPER.lock().unwrap_or_else(std::sync::PoisonError::into_inner).running = true;
        let (accepted, accepted_child) = std::sync::mpsc::channel();
        let panicked = std::panic::catch_unwind(|| {
            ...
            let _reaper = report_reaper_warnings(reaper, vec!["panicking warning".to_owned()], |_warning| {
                ...
                reap_later(child).expect("the handoff observes the reporting reaper as running");
                accepted.send(child_id).expect("the parent waits for the raced handoff");
        ...
        let child_id = accepted_child
            .recv_timeout(Duration::from_secs(1))
            .expect("the warning callback accepted a child during the handoff window");
