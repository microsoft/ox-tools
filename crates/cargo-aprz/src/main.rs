...
//!    ```
//!
//! 6. If a crate gets flagged and you want to acknowledge the finding so it stops failing your build,
...
//! 1. `--github-token`
//! 2. `GITHUB_TOKEN`
//! 3. when `--github-token-from-gh` is present, the token reported by
...
#![doc(html_logo_url = "https://media.githubusercontent.com/media/microsoft/ox-tools/refs/heads/main/crates/cargo-aprz/logo.png")]
#![doc(html_favicon_url = "https://media.githubusercontent.com/media/microsoft/ox-tools/refs/heads/main/crates/cargo-aprz/favicon.ico")]
...
// miri cannot execute mimalloc's foreign allocation functions, so it falls back
// to the system allocator when running under miri.
#[cfg(not(miri))]
#[global_allocator]
