...
fn sync_entry(source: &Utf8Path, destination: &Utf8Path, reflinks: &Reflinks) -> Result<()> {
    ...
    if src_meta.is_dir() {
        ...
        } else if !destination_is_directory(destination) {
...
fn readers_have_same_contents(left: impl Read, right: impl Read) -> std::io::Result<bool> {
    // #[gamma::skip(arith.mul_to_add, literal.int_increment, literal.int_to_one, literal.int_decrement, reason = "buffer capacity changes only how identical byte streams are chunked; fill_buf/consume explicitly aligns independent chunk boundaries")]
...
fn destination_is_directory(path: &Utf8Path) -> bool {
...
mod tests {
    ...
    fn source_collection_obeys_hidden_ignore_pruning_and_link_boundaries() {
        ...
        let included = collect_source_entries(
            &from,
            &skip,
            CopyOptions {
                copy_ignored: true,
        ...
        .expect("ignored-file collection");
    ...
    fn synchronization_predicates_distinguish_every_file_state() {
        ...
        assert!(destination_is_directory(&to.join("directory")));
        assert!(!destination_is_directory(&to.join("same")));
        ...
        assert!(ignore_not_found(io::Error::from(ErrorKind::NotFound)).is_ok());
    ...
    impl Read for ScheduledReader<'_> {
        fn read(&mut self, destination: &mut [u8]) -> io::Result<usize> {
            ...
            self.turn += 1;
            let count = scheduled.min(destination.len()).min(self.bytes.len() - self.offset);
