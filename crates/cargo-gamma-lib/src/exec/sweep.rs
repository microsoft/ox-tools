...
/// sweep runs unchanged. The caller also permits a probe only when neither its binary nor anything
/// before it can produce a resource verdict: a kill must not bypass a timeout or memory limit,
/// because those outcomes score differently.
fn probe(
    work: &Workspace,
    ordinal: u32,
    binary: &TestBinary,
...
/// Whether moving work through `last` to the front cannot bypass a resource verdict.
///
/// A cached or learned killer may still save the launches before it when all of them are unbounded.
/// Once any binary has a timeout, stall budget, or memory ceiling, canonical order is load-bearing:
/// reaching the hinted kill first would turn an undetected resource outcome into a detected kill.
fn score_neutral_prefix(reachable: &[&TestBinary], last: usize, timeout_multiplier: Option<f64>, sweep: Sweep<'_>) -> bool {
    reachable[..=last].iter().all(|binary| {
        binary.budget_for(timeout_multiplier, sweep.timeout_floor).is_none()
            && sweep.stall.budget.is_none()
            && (!sweep.meter || binary.memory.is_none())
    })
}
...
///
/// When an earlier run recorded which test caught this mutant, that one test is tried first only
/// when doing so cannot bypass a resource verdict. It is a guess and it is checked, never believed:
/// see [`probe`].
#[cfg(test)]
fn judge(
    work: &Workspace,
...
fn judge_ordered(
    ...
    // The hint has to name a binary this mutant can actually reach. One that does not is a map
    // written before the test packages were narrowed, or before the test moved, and running it
    // would judge the mutant against a binary the run has excluded.
    if let Some(hint) = hint
        && let Some((position, binary)) = reachable
            .iter()
            .enumerate()
            .find(|(_position, binary)| hint.names(&binary.package, &binary.target))
        && score_neutral_prefix(reachable, position, timeout_multiplier, sweep)
    ...
    let preferred = file_hint.and_then(|hint| {
        reachable
            .iter()
            .copied()
            .enumerate()
            .find(|(position, binary)| {
                hint.names(&binary.package, &binary.target) && score_neutral_prefix(reachable, *position, timeout_multiplier, sweep)
            })
            .map(|(_position, binary)| binary)
    });

    // An exact killer already returned above, and a score-neutral file-local one is pinned first by
    // `preferred`. Nothing beyond that reorders the rest.
    let ordered = preferred.into_iter().chain(
        reachable
            .iter()
            .copied()
            .filter(|binary| preferred.is_none_or(|first| !core::ptr::eq(*binary, first))),
    );
    ...
    for binary in ordered {
        ...
        if let CensusSelection::Hinted(names) = &selection
            && score_neutral_prefix(core::slice::from_ref(&binary), 0, timeout_multiplier, sweep)
...
mod tests {
    ...
    #[test]
    #[cfg(unix)]
    fn file_local_killer_order_reduces_launches_during_the_same_sweep() {
        ...
        assert_eq!(spent.launches, 3, "the second mutant should try the observed killing binary first");
        assert_eq!(spent.probes, 0, "file-local ordering is not a per-mutant probe");
    }
