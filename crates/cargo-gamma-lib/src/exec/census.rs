...
mod tests {
    ...
    #[test]
    fn durable_reach_clusters_sort_by_normalized_site_before_occurrence() {
        let binary = binary("/t/a");
        let mut census = Census::default();
        let _previous = census.binaries.insert(
            binary.path.clone(),
        ...
        let order: Vec<(&str, u32)> = hints
            .reach
            .iter()
            .map(|cluster| (cluster.site.normalized_text.as_str(), cluster.site.occurrence))
            .collect();
        assert_eq!(order, vec![("a + 1", 0), ("a + 1", 1), ("z + 1", 0)]);
    }
