...
fn output_matches(invocation: &RustcInvocation, artifact: &Utf8Path) -> bool {
    ...
    let matches = |candidate: &str| {
        candidate
            .strip_prefix(invocation.crate_name.as_str())
...
mod tests {
    ...
    #[test]
    fn a_lib_prefixed_test_executable_matches_its_capture() {
        let directory = tempfile::tempdir().expect("capture directory");
        let out = Utf8PathBuf::from_path_buf(directory.path().join("target").join("deps")).expect("utf-8 target");
        let invocation = RustcInvocation {
            crate_name: "library_tests".to_owned(),
            crate_types: vec!["bin".to_owned()],
            test: true,
            source: "tests/library_tests.rs".into(),
            out_dir: out.clone(),
            extra_filename: "-hash".to_owned(),
            externs: Vec::new(),
            opaque_extern: false,
        };
        let executable = out.join(format!("library_tests-hash{}", std::env::consts::EXE_SUFFIX));

        assert!(output_matches(&invocation, &executable));
    }
    ...
    fn unit_test_target_kinds_disable_exact_cargo_build_selection() {
        ...
        for (name, kind) in [("library", "lib"), ("binary", "bin"), ("example", "example")] {
            ...
            let artifact = out.join(format!("{name}-{name}hash{}", std::env::consts::EXE_SUFFIX));
            assert!(
