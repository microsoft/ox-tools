...
pub fn build(plan: &Plan, thresholds: Thresholds, run: Option<RunInfo>) -> Result<Report> {
    ...
    for file in &plan.files {
        ...
        {
            return Err(error!(
                "`{}` changed after its mutants were discovered; refusing to combine verdicts and source from different generations",
                file.path
            ));
        }
...
";
    ...
    fn a_report_refuses_source_changed_after_discovery() {
        ...
        assert!(error.to_string().contains("changed after its mutants were discovered"), "{error}");
    }
