...
/// that renders a name wrongly, drops the alias column, or stops escaping a pipe can agree with
/// both sides of that comparison and be copied into the published reference.
///
/// These expectations were typed by hand from what the tables are supposed to say, so a rendering
/// change has to be defended here rather than copied mechanically. One representative row from
/// each generated block keeps the oracle independent without duplicating the complete tables.
#[test]
...
///
/// This is deliberately a rustfmt-shaped scanner rather than a Rust parser. Repository source is
/// formatted before review, leaving the visibility and function qualifiers plus `fn NAME` on one
/// line in the order recognized below; attributes and a multi-line parameter list may occupy other
/// lines without changing that declaration line. If the repository adopts another declaration
/// shape, this scanner must be extended with that convention rather than silently treating it as
/// outside the environment-write policy.
...
///
/// Rustfmt keeps file-bearing `mod NAME;` declarations on one line. This scanner accepts the four
/// visibility forms used by the workspace and reads only that first identifier; inline module
/// bodies are harmlessly included because the policy needs only the set of possible local roots.
...
/// widens nothing outside this crate. Rustfmt leaves `pub use` and its first path segment on one
/// line even when a brace list wraps, which is the only source shape this policy needs to classify.
...
/// No supported public facade re-exports by glob, anywhere in the tool's crates.
///
/// A `pub use module::*` promises whatever that module happens to hold *next*, which makes adding a
/// `pub` item to an implementation module a silent change to another crate's supported surface.
/// The documentation-hidden `internals` facade is excluded because automatic mirroring is its
/// explicit test-only contract and keeps the implementation modules private at the crate root.
///
...
fn no_public_facade_re_exports_by_glob() {
    ...
    for path in gamma_crates() {
        for source in walk(&path.join("src")) {
            ...
            for (number, line) in text.lines().enumerate() {
                if line.trim() == "pub use crate::$name::*;" {
                    continue;
                }
