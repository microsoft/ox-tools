...
///
/// `cargo-nextest` is not optional here — every gate in this repository runs through it — so its
/// absence is a broken environment rather than a reason to skip.
#[test]
fn a_run_through_nextest_reaches_the_same_verdicts_as_one_through_the_binaries() {
    ...
    assert!(
        ...
        "`cargo-nextest` must be installed to run this suite; every gate in this repository uses it"
    );
