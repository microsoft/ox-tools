...
reach observations collected before the deadline are retained as checked hints. The named cases run
first only when the binary has no resource bound whose outcome they could bypass, and any result
other than a kill falls back to the whole binary. A malformed, empty, absent, or failed sample
discards that binary's census. Unmeasured reach always means “run everything,” never “nothing was
reached.”
...
Prior killers influence scheduling and, when no resource-bound binary precedes them, test order. A
previous killer is only a hint: when its test is rerun, it must convict again. If it does not, normal
testing continues. A hint cannot move a kill ahead of a timeout or memory limit, so stale hints can
waste work but cannot settle or change a verdict.
