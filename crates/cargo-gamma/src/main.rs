...
//!
//! Why is it called `cargo-gamma` you might ask? Because [gamma radiation](https://en.wikipedia.org/wiki/Gamma_ray)
//! induces genetic mutations (ref [The Hulk](https://en.wikipedia.org/wiki/hulk)).
//!
//! ## Getting started
//!
...
//!
//! [Mutators](docs/MUTATORS.md) is the full reference of all mutators, with a table per family, every
...
//!
//! A [mutator preset](docs/MUTATORS.md#mutator-presets) groups the catalog by what a mutant disturbs. For example,
...
//! killer first. Killer tests are hints, not cached verdicts: every mutant is executed again before it
//! contributes to the score. Reusing unviability requires the same compiler, Cargo configuration, and
...
//! [`cargo-test-args`](docs/CONFIG.md) configuration key takes, which a file cannot write as a trailing
//! `--`.
//!
...
//! test target of their own and naming it to [`--exclude-test`](docs/CMDLINE.md#full-option-reference): the run
...
//!
//! The [mutation-testing-elements](https://github.com/stryker-mutator/mutation-testing-elements)
...
//! 2. **Cluster by function or module:** Multiple survivors in the same function usually point to a
//!    single missing test scenario rather than multiple independent test gaps.
//! 3. **Prefer public contract assertions:** Write tests against the public API to verify observable
//!    behavior rather than writing tests that tightly couple to internal private implementation details.
//! 4. **Distinguish non-functional and cache code:** If a survivor alters an internal cache, memoization,
...
//! genuinely allowed to allocate this much can be taken out of the run for good with `cargo gamma
//! suppress`, which treats `outofmem` as eligible by default alongside `timeout`.
//!
...
//!
//! - **Killer hints** — which test caught each mutant. The named test is run first rather than
//!   believed, so a hint that has gone stale costs one filtered test process before falling back
//!   to the standard whole-binary sweep.
...
//! checked hint: gamma tries it first, accepts an actual failure as a kill, and otherwise falls back to
//! the whole binary. A binary that cannot be listed or whose sample fails is not narrowed at all.
//!
//! ### Incremental runs
//!
